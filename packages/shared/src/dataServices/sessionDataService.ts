...
function normalizeDateBoundary(
  ...
  const trimmedValue = value.split("T")[0] ?? value;
  ...
  if (boundary === "end") {
    ...
    const nextMonth = `${parsed.getMonth() + 1}`.padStart(2, "0");
    const nextDay = `${parsed.getDate()}`.padStart(2, "0");
...
export async function listSessions(filters?: {
  ...
  const normalizedFrom = normalizeDateBoundary(filters?.from, "start");
  const normalizedTo = normalizeDateBoundary(filters?.to, "end");
  ...
  if (normalizedFrom || normalizedTo) {
    query = query.gte("start_at", normalizedFrom ?? new Date().toISOString());
    if (normalizedTo) {
      query = query.lte("start_at", normalizedTo);
    }
  }
  ...
  if (normalizedFrom || normalizedTo) {
    results = results.filter((r: any) => {
      ...
      if (normalizedTo && start >= new Date(`${normalizedTo}T00:00:00`)) {
        return false;
      }
      return true;
    });
  ...
  return { sessions: results, error: null };
}
