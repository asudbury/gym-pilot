...
export async function getUserSessions(userId: string): Promise<{ data: UserSession[] | null; error: any | null }> {
  return getItems<UserSession>(tableName, userId);
}

export async function getUserSession(id: string, userId: string): Promise<{ data: UserSession | null; error: any | null }> {
