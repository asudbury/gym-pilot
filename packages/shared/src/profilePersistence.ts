...
function normalizeProfileRoles(roles: unknown): UserRole[] {
  if (Array.isArray(roles)) {
    return roles.filter((role): role is UserRole => typeof role === 'string' && role.length > 0) as UserRole[]
  }

  return []
}

function normalizeProfileSnapshotRow(profile: Record<string, unknown>, roleOverride?: UserRole[]): SupabaseProfileSnapshot {
  return {
    friendlyName: typeof profile.friendly_name === 'string' ? profile.friendly_name : null,
    applicationName: typeof profile.application_name === 'string' ? profile.application_name : null,
    ...
    gymClubId: typeof profile.gym_club_id === 'string' ? profile.gym_club_id : null,
    accountTier: normalizeProfileAccessTier(profile.account_tier),
    accessEndsAt: typeof profile.access_ends_at === 'string' ? profile.access_ends_at : null,
    isFrozen: Boolean(profile.is_frozen),
    lastLoggedInAt: typeof profile.last_logged_in_at === 'string' ? profile.last_logged_in_at : null,
    previousLastLoggedInAt: typeof profile.previous_last_logged_in_at === 'string' ? profile.previous_last_logged_in_at : null,
    mustChangePassword: Boolean(profile.must_change_password),
    termsAccepted: Boolean(profile.terms_accepted),
    termsAcceptedAt: typeof profile.terms_accepted_at === 'string' ? profile.terms_accepted_at : null,
    roles: roleOverride ?? normalizeProfileRoles(profile.roles),
    trainerId: typeof profile.trainer_id === 'string' ? profile.trainer_id : null,
...
export async function loadSupabaseProfileSnapshot(userId?: string): Promise<SupabaseProfileSnapshot> {
  ...
  const snapshotPromise = (async () => {
    ...
    const { data, error } = await client.from('gym_pilot_profile').select('*').eq('user_id', resolvedUserId).maybeSingle()
    if (error) {
      logger.warn('[Supabase] Could not load profile snapshot', error)
      return createEmptyProfileSnapshot()
    }

    const baseSnapshot = data ? normalizeProfileSnapshotRow(data as Record<string, unknown>) : createEmptyProfileSnapshot()

    await persistSupabaseProfileSnapshotToLocalCache(resolvedUserId, baseSnapshot)
    return baseSnapshot
  })()
...
export async function loadSupabaseProfileAccessState(userId?: string): Promise<{ accountTier: SupabaseAccessTier; accessEndsAt: string | null; isFrozen: boolean; isBlocked: boolean; blockReason: 'frozen' | 'expired' | null }> {
  ...
  return {
    ...
    isBlocked: snapshot.isFrozen || Boolean(snapshot.accessEndsAt && new Date(snapshot.accessEndsAt).getTime() < Date.now()),
    blockReason: snapshot.isFrozen ? 'frozen' : snapshot.accessEndsAt && new Date(snapshot.accessEndsAt).getTime() < Date.now() ? 'expired' : null,
  }
}
...
export async function loadSupabaseProfileEmail(userId?: string): Promise<string | null> {
  const snapshot = await loadSupabaseProfileSnapshot(userId)
...
export async function loadSupabaseProfileRoles(userId: string): Promise<UserRole[]> {
  ...
  return (Array.isArray(data) ? data : []).map((row) => (row && typeof row === 'object' ? (row as Record<string, unknown>).role : undefined)).filter((role): role is UserRole => typeof role === 'string' && normalizeUserRoles([role]).length > 0)
}
...
export async function saveSupabaseProfileName(friendlyName: string | null, userId?: string) {
  const client = getSupabaseClient()
  if (!client) {
    return
  }
  ...
  const { error } = await client.from('gym_pilot_profile').upsert({ user_id: resolvedUserId, friendly_name: friendlyName }, { onConflict: 'user_id' })
  if (error) {
...
export async function saveSupabaseProfileEmail(email: string | null, userId?: string) {
  const client = getSupabaseClient()
  if (!client) {
    return
  }
  ...
  const { error } = await client.from('gym_pilot_profile').upsert({ user_id: resolvedUserId, email }, { onConflict: 'user_id' })
  if (error) {
...
export async function saveSupabaseApplicationName(applicationName: string | null) {
  ...
  const { error } = await client.from('gym_pilot_profile').upsert({ user_id: resolvedUserId, application_name: applicationName }, { onConflict: 'user_id' })
  if (error) {
...
export async function saveSupabaseGymBrand(gymBrand: string | null) {
  ...
  const { error } = await client.from('gym_pilot_profile').upsert({ user_id: resolvedUserId, gym_brand: gymBrand }, { onConflict: 'user_id' })
  if (error) {
...
export async function saveSupabaseGymName(gymName: string | null, gymBrand?: string | null) {
  ...
  const { error } = await client.from('gym_pilot_profile').upsert({ user_id: resolvedUserId, gym_name: gymName, gym_brand: gymBrand ?? null }, { onConflict: 'user_id' })
  if (error) {
...
export async function saveSupabaseProfileAccessSettings(accountTier: string | null, accessEndsAt: string | null, isFrozen: boolean, userId?: string) {
  ...
  const { error } = await client.from('gym_pilot_profile').upsert({ user_id: resolvedUserId, account_tier: accountTier, access_ends_at: accessEndsAt, is_frozen: isFrozen }, { onConflict: 'user_id' })
  if (error) {
...
export async function saveSupabaseProfileFlag(_flag: 'must_change_password', value: boolean, userId?: string) {
  const client = getSupabaseClient()
  if (!client) {
    return
  }
  ...
  const { error } = await client.from('gym_pilot_profile').upsert({ user_id: resolvedUserId, must_change_password: value }, { onConflict: 'user_id' })
  if (error) {
...
export async function saveSupabaseProfileRoles(
  ...
  const roleRows = normalizedRoles.map((role) => ({ user_id: resolvedUserId, role }))
...
export async function saveSupabaseProfileTermsAcceptance(accepted: boolean, userId?: string) {
  ...
  const { error } = await client.from('gym_pilot_profile').upsert({ user_id: resolvedUserId, terms_accepted: accepted, terms_accepted_at: accepted ? new Date().toISOString() : null }, { onConflict: 'user_id' })
  if (error) {
