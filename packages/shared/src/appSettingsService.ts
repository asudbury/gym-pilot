...
import { TableNames } from "./dataServices/tableNames";
import { logger } from "./logging";
import { getSupabaseClient } from "./supabase";

export async function loadAppSettings(): Promise<
  Record<string, AppSettingValue>
> {
  ...
  return buildAppSettingsMap(
    (data ?? []) as Array<
      Partial<{ setting_key: AppSettingKey; setting_value: AppSettingValue }>
    >,
  );
}

export async function loadAppSetting(
  key: AppSettingKey,
...
export async function saveAppSettings(
  ...
  await Promise.all(
    Object.entries(settings).map(([key, value]) =>
      saveAppSetting(key as AppSettingKey, value),
    ),
  );
}
