...
describe('session workout persistence payloads', () => {
  ...
  it('persists workout items in the order defined by their sort order values', () => {
    const payloads = buildWorkoutItemsPersistencePayloads({
      ...
      userId: 'user-1',
      workoutItems: [
        { id: 'item-2', category: 'exercise', exerciseName: 'Run', sortOrder: 2 } as any,
        { id: 'item-1', category: 'exercise', exerciseName: 'Squat', sortOrder: 0 } as any,
        { id: 'item-3', category: 'exercise', exerciseName: 'Plank', sortOrder: 1 } as any,
      ],
    })

    expect(payloads.map((payload) => payload.exercise_name)).toEqual(['Squat', 'Plank', 'Run'])
