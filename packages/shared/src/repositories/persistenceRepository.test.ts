...
describe("createPersistenceRepository", () => {
  it("loads from the remote source when enabled and the local source is unavailable", async () => {
    ...
    const loadRemote = vi
      .fn()
      .mockResolvedValue({ found: true, value: { ok: true } });
    ...
    await expect(repository.load("demo", { ok: false })).resolves.toEqual({
      ok: true,
    });
    expect(loadLocal).toHaveBeenCalledWith("demo", expect.anything());
    expect(loadRemote).toHaveBeenCalledWith("demo");
  });

  it("prefers the local value and skips remote load when local data exists", async () => {
    const loadLocal = vi.fn().mockResolvedValue({ ok: true });
    const loadRemote = vi
      .fn()
      .mockResolvedValue({ found: true, value: { ok: false } });
