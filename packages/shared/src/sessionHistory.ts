...
export async function loadSessionHistoryEntries(userId?: string): Promise<SessionHistoryEntry[]> {
  ...
  try {
    ...
    return (data ?? []).map((row) => mapSessionHistoryEntryFromSupabase(row as SupabaseSessionHistoryRow))
  } catch (error) {
    logger.warn('[Supabase] Could not load session history entries', error)
    return []
  }
...
export async function saveSessionHistoryEntry(entry: SessionHistoryEntry, userId?: string): Promise<SessionHistoryEntry[]> {
  ...
  try {
    const { data, error } = await client.from(getSessionHistoryTableName()).upsert({
      ...payload,
      id: entry.id,
...
export async function deleteSessionHistoryEntry(entryId: string, userId?: string): Promise<void> {
  const client = getSupabaseClient()
...
export async function upsertSessionHistoryEntry(entry: SessionHistoryEntry, userId?: string): Promise<SessionHistoryEntry[]> {
  return saveSessionHistoryEntry(entry, userId)
}

export async function removeSessionHistoryEntry(entryId: string, userId?: string): Promise<void> {
...
export async function createSession(payload: Record<string, unknown>, userId?: string) {
  const client = getSupabaseClient()
  ...
  const { error } = await client.from(getSessionTableName()).insert({ ...payload, user_id: resolvedUserId })
...
export async function bookSession(payload: Record<string, unknown>, userId?: string) {
  const client = getSupabaseClient()
  ...
  const { error } = await client.from(getSessionBookingTableName()).insert({ ...payload, user_id: resolvedUserId })
...
export async function recordSession(payload: Record<string, unknown>, userId?: string) {
  const client = getSupabaseClient()
...
export async function cancelBooking(bookingId: string, userId?: string) {
  ...
  const { error } = await client.from(getSessionBookingTableName()).delete().eq('id', bookingId).eq('user_id', resolvedUserId)
...
export async function listSessions(userId?: string) {
  ...
  const { data, error } = await client.from(getSessionTableName()).select('*').eq('user_id', resolvedUserId)
...
export async function listBookings(userId?: string) {
  ...
  const { data, error } = await client.from(getSessionBookingTableName()).select('*').eq('user_id', resolvedUserId)
