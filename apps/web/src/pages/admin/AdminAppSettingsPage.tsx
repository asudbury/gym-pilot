...
const SETTING_FIELDS: Array<{ key: AppSettingKey; label: string; type: 'boolean' | 'text' }> = [
  ...
  { key: 'error_logging_enabled', label: 'Error logging enabled', type: 'boolean' },
  { key: 'audit_logging_enabled', label: 'Audit logging enabled', type: 'boolean' },
  { key: 'log_level', label: 'Log level', type: 'text' },
  { key: 'user_activity_logging_enabled', label: 'User activity logging', type: 'boolean' },
  { key: 'activities_my_logging_enabled', label: 'Activities my logging', type: 'boolean' },
]
...
export function AdminAppSettingsPage() {
  ...
  const fieldRows = useMemo(() => SETTING_FIELDS.map((field) => ({ ...field, value: settings[field.key] })), [settings])
      ...
      >
        <div className="space-y-6">
          <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void handleSave() }}>
            {fieldRows.map((field) => (
              <label key={field.key} className="flex items-start justify-between gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex flex-col">
                  <span className="text-sm font-semibold text-slate-900">{field.label}</span>
                  <span className="text-sm text-slate-600">{field.type === 'boolean' ? 'Toggle this setting on or off.' : 'Use a short message or value for this setting.'}</span>
                </div>
