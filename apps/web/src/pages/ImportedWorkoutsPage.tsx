...
export function ImportedWorkoutsPage() {
  ...
  const columnDefs: ColDef<ImportedWorkout>[] = useMemo(() => [
    { field: "display_name", headerName: "Name", filter: true, floatingFilter: true },
    { field: "start_date", headerName: "Start Date", filter: 'agDateColumnFilter' },
    { field: "duration", headerName: "Duration (mins)", valueSetter: (params: ValueSetterParams) => {
        params.data.duration = Number(params.newValue);
        return true;
    }},
    { field: "energy", headerName: "Energy", valueSetter: (params: ValueSetterParams) => {
  ...
  return (
    <PageLayout className="max-w-6xl">
      ...
      >
        ...
        <div className="mb-4">
            <button className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition-all duration-200 hover:border-slate-400 hover:bg-slate-50 hover:font-semibold hover:shadow-sm" onClick={onExportClick}>Export as CSV</button>
        </div>
