...
export const SpreadsheetImportPreviewDetailPage: React.FC = () => {
  ...
  const [previewItems, setPreviewItems] = useState<SpreadsheetWorkoutImportPreviewItem[] | null>(null)
  ...
  if (!user?.id) {
    return (
      <PageLayout className="gap-6">
        <PageCardLayout title="Preview detail" description="You must be logged in to edit preview items." icon="apple">
  ...
  if (!previewItems || Number.isNaN(itemIndex) || itemIndex < 0 || itemIndex >= previewItems.length) {
    return (
      <PageLayout className="gap-6">
        <PageCardLayout title="Preview detail" description="No preview item found." icon="apple">
          <div className="text-sm">No preview item available. Return to the preview list.</div>
          <div className="mt-4">
  ...
  const saveAndBack = () => {
    try {
      localStorage.setItem('gym-pilot:spreadsheetImportPreview', JSON.stringify(previewItems))
    } catch {
      // ignore
    }
  ...
  return (
    <PageLayout className="gap-6">
      <PageCardLayout title={`Edit imported session ${itemIndex + 1}`} description="Adjust date, duration and review exercises." icon="apple">
        <div className="rounded border border-slate-200 p-3 dark:border-slate-700">
          <label className="flex items-center gap-2">
            <span className="text-sm">Date</span>
            <input type="date" value={item.payload.session.start_at.slice(0, 10)} onChange={(e) => {
              const nextDate = e.target.value
              if (!nextDate) return
              const nextStartAt = new Date(`${nextDate}T00:00:00`).toISOString()
              setPreviewItems((current) => current ? updateSpreadsheetImportPreviewItem(current, itemIndex, { payload: updateSpreadsheetImportPreviewDate(current[itemIndex].payload, nextStartAt) }) : current)
          ...
          <label className="mt-3 flex items-center gap-2">
            ...
            <input type="number" min="0" value={item.payload.session.duration_minutes ?? ''} onChange={(e) => {
              const nextValue = e.target.value
              const parsedValue = nextValue === '' ? null : Number(nextValue)
              const nextDuration = Number.isNaN(parsedValue) ? null : parsedValue
              setPreviewItems((current) => current ? updateSpreadsheetImportPreviewItem(current, itemIndex, { payload: updateSpreadsheetImportPreviewDuration(current[itemIndex].payload, nextDuration) }) : current)
          ...
          <div className="mt-3">
            ...
            <ul className="mt-2 list-disc pl-5 text-xs text-slate-600 dark:text-slate-400">
              {item.payload.workout_items.map((wi) => (
                <li key={`${wi.exercise_name}-${wi.item_index}`}>{wi.exercise_name}</li>
              ))}
            </ul>
          </div>

          <div className="mt-4 flex gap-2">
            <Button tone="default" onClick={() => navigate('/apple-fitness/import-spreadsheet/preview')}>Cancel</Button>
            <Button tone="emerald" onClick={saveAndBack}>Save</Button>
          </div>
        </div>
      </PageCardLayout>
    </PageLayout>
