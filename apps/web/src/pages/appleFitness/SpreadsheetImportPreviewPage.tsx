...
export const SpreadsheetImportPreviewPage: React.FC = () => {
  ...
  const [spreadsheetImportError, setSpreadsheetImportError] = useState<
    string | null
  >(null)
  const [spreadsheetImportMessage, setSpreadsheetImportMessage] = useState<
    string | null
  >(null)
  ...
  const parseSpreadsheetImportContent = (
    rawContent: string,
  ): SpreadsheetImportContent => {
    ...
    try {
      ...
      if (persisted && persisted.length > 0) {
        ...
        if (payloads.length === 0) {
          throw new Error(
            'No sessions could be detected in the pasted spreadsheet content.',
          )
        }

        setPreviewPayloads(
          payloads.map((payload) => ({ payload, selected: true })),
        )
      }
      ...
      setSpreadsheetImportError(
        error instanceof Error
          ? error.message
          : 'An unexpected error occurred while previewing spreadsheet sessions.',
      )
      setSpreadsheetImportMessage(null)
      setPreviewPayloads([])
    }
  ...
  return (
    <PageLayout className="gap-6">
      <PageCardLayout
        ...
        description="Review each detected session before confirming the import."
        icon="apple"
      >
        <div className="flex flex-wrap items-center gap-3">
          <Button
            onClick={() => navigate('/apple-fitness/import-spreadsheet')}
            tone="default"
          >
            Back to input
          </Button>
          ...
          <div className="flex flex-col gap-3">
            ...
            {spreadsheetImportMessage && (
              <div className="flex flex-col gap-3">
                <StatusMessageNotification
                  tone="info"
                  message={spreadsheetImportMessage}
                />
              ...
              <div className="rounded-md border border-slate-200 bg-white p-3 text-sm dark:border-slate-700 dark:bg-slate-950">
                <div className="mb-2 flex items-center justify-between">
                  <p className="font-medium text-slate-900 dark:text-slate-100">
                    Preview ({previewPayloads.length} session
                    {previewPayloads.length === 1 ? '' : 's'})
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  {previewPayloads.map((previewItem, index) => {
                    ...
                    return (
                      <div
                        key={`${payload.session.session_id}-${index}`}
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-2">
                                <label className="flex items-center gap-1 text-sm font-medium text-slate-900 dark:text-slate-100">
                                  ...
                                  <input
                                    type="date"
                                    value={payload.session.start_at.slice(
                                      0,
                                      10,
                                    )}
                                    onChange={(event) => {
                                            ...
                                            payload:
                                              updateSpreadsheetImportPreviewDate(
                                                currentItems[index]?.payload ??
                                                  ({} as Parameters<
                                                    typeof updateSpreadsheetImportPreviewDate
                                                  >[0]),
                                                nextStartAt,
                                              ),
                                          },
                              ...
                              <div className="flex items-center gap-2">
                                <Button
                                  onClick={() =>
                                    navigate(
                                      `/apple-fitness/import-spreadsheet/preview/${index}`,
                                    )
                                  }
                                  tone="default"
                                >
                                  ...
                                  onChange={(event) => {
                                    ...
                                    const parsedValue =
                                      nextValue === ''
                                        ? null
                                        : Number(nextValue)
                                    const nextDuration = Number.isNaN(
                                      parsedValue,
                                    )
                                      ? null
                                      : parsedValue
                                        ...
                                        index,
                                        {
                                          payload:
                                            updateSpreadsheetImportPreviewDuration(
                                              currentItems[index]?.payload ??
                                                ({} as Parameters<
                                                  typeof updateSpreadsheetImportPreviewDuration
                                                >[0]),
                                              nextDuration,
                                            ),
                                        },
                            ...
                            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                              {typeof payload.session.metadata?.summary ===
                                'string' && payload.session.metadata.summary
                                ? payload.session.metadata.summary
        ...
        <div className="mt-4 flex items-center gap-3 justify-start">
          <Button
            onClick={() =>
              navigate('/apple-fitness/import-spreadsheet/confirm')
            }
            tone="emerald"
            disabled={previewPayloads.length === 0}
