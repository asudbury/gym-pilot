...
export function PlanBuilderWorkspace({
  ...
  const columnDefs = useMemo<ColDef<PlanGridRow>[]>(() => {
    ...
    return defs
  }, [activeRows.length, onCellChange, onMoveRow, onRemoveRow])
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-3 sm:p-4">
      <div className="space-y-3">
        ...
        <div
          className={`overflow-hidden rounded-2xl border ${isDropActive ? 'border-emerald-400 bg-emerald-50/70' : 'border-slate-200 bg-white'}`}
          onDragEnter={(event) => {
            event.preventDefault()
        ...
        >
          ...
          <div className={`ag-theme-quartz h-96 sm:h-105'} w-full`}>
