      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {folderName !== 'No folder' && (
            <button type="button" onClick={() => onDeleteFolder(folderName)} className="rounded-lg border border-rose-200 bg-rose-50 px-2 py-1 text-xs font-medium text-rose-700 transition hover:bg-rose-100">
              Delete folder
            </button>
          )}
            ...
            <div key={link.id} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                  ...
                  <select value={normalizeFolderName(link.folder ?? '') || 'No folder'} onChange={(event) => onMoveLink(link, event.target.value)} className="mt-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
                ...
                <button type="button" onClick={() => onOpenLink(link)} className={getToneClass('default', 'rounded-lg px-3 py-2 text-sm font-medium')}>
                  Open
                </button>
                <button type="button" onClick={() => onRemoveLink(link)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-100">
                  Remove
                </button>
              </div>
            </div>
