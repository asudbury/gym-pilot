...
describe('planBuilderTransitions', () => {
  it('creates a tab state with a new tab and selected exercise', () => {
    const state = resolvePlanBuilderTabState(
      {
        tabs: [createBlankTab('Day 1')],
        activeTabId: 'day-1',
        selectedExerciseId: 'ex1',
        selectedExerciseName: 'Bench press',
        personNamesInput: 'Team',
      },
      'Day 2',
    )
  ...
  it('resolves reset state and link rows', () => {
    ...
    const linkRows = resolvePlanBuilderLinkRows(
      [
  ...
  it('hydrates from existing plan sessions', () => {
    const state = resolvePlanBuilderHydrationState({
      planName: 'My plan',
      planSessions: [],
    })
