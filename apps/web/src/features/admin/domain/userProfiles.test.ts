...
import { createInitialProfileDraft, mapProfileRow, resolveTrainerOptions, toggleRoleSelection } from './userProfiles'
...
describe('toggleRoleSelection', () => {
  it('adds and removes roles', () => {
    expect(toggleRoleSelection(['client'], 'trainer')).toEqual(['client', 'trainer'])
    expect(toggleRoleSelection(['client', 'trainer'], 'trainer')).toEqual(['client'])
  })
})
