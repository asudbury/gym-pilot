...
export function resolveUserActivityViewModel(
  ...
  const profile = profileRow
    ? {
        ...
        name:
          typeof profileRow.friendly_name === 'string' &&
        ...
        roles: getDisplayRoles(
          Array.isArray(profileRow.roles) ? profileRow.roles : undefined,
        ),
        lastLoggedInAt:
          typeof profileRow.last_logged_in_at === 'string'
            ? profileRow.last_logged_in_at
            : null,
        previousLastLoggedInAt:
          typeof profileRow.previous_last_logged_in_at === 'string'
            ? profileRow.previous_last_logged_in_at
            : null,
      }
    : null
  ...
  return {
    ...
    activityRows: activityRows.map((row) => ({
      id: String(row.id ?? ''),
      eventType:
        typeof row.event_type === 'string' ? row.event_type : 'activity',
      createdAt: typeof row.created_at === 'string' ? row.created_at : '',
      eventData:
        row.event_data &&
        typeof row.event_data === 'object' &&
        !Array.isArray(row.event_data)
