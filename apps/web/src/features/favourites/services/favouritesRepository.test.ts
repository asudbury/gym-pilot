...
describe('favourites repository', () => {
  it('loads favourites from the repository fallback when the key is not supported', async () => {
    ...
    await expect(repository.load('other-key', { favorites: [], folders: [] })).resolves.toEqual({ favorites: [], folders: [] })
  })
  ...
  it('saves favourites through the repository abstraction', async () => {
    ...
    const repository = createPersistenceRepository({
      loadLocal: vi.fn().mockResolvedValue({ favorites: [{ id: '1', label: 'Home', path: '/home' }], folders: [] }),
      saveLocal,
    ...
    await repository.save('gym-pilot.favorites', { favorites: [{ id: '1', label: 'Home', path: '/home' }], folders: [] })

    expect(saveLocal).toHaveBeenCalledWith('gym-pilot.favorites', { favorites: [{ id: '1', label: 'Home', path: '/home' }], folders: [] })
